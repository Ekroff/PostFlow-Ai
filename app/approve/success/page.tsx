export default function ApproveSuccessPage({
  ...
  const approved = searchParams.action === 'approved';
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center max-w-md shadow-sm">
        <div className="text-5xl mb-4">{approved ? '✅' : '↩️'}</div>
        <h1 className="text-xl font-bold text-gray-900 mb-2">
          {approved ? 'Post approved!' : 'Changes requested'}
        </h1>
        <p className="text-gray-500 text-sm">
          {approved
            ? 'The post has been approved and will publish at the scheduled time.'
            : 'The author has been notified to make revisions.'}
        </p>
        <a
          href="/app/queue"
